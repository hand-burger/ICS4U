...
void scanCube()
{
	...
	for (size_t i = 0; i < faceScans.size(); i++)
	{
		...
		//for (int k = 0; k < 6; k++)
		//{
		//	// Loop over each color position
		//	for (int l = 0; l < 9; l++)
		//	{
		//		// Now compare faceMasks[k] to the color at colorPoints[l]
		//		cout << l << endl;
		//		Scalar color = faceMasks[k].at<uchar>(Point(colorPoints[l].x, colorPoints[l].y));

		//		// Now if this color is white
		//		if (color[0] == 255)
		//		{
	...
	//	{
	//		// Yellow mask
	//		inRange(faceScans[i], Scalar(24, 132, 94), Scalar(37, 255, 123), faceMasks[l]);
	//		dilate(faceMasks[i], faceMasks[l], kernel);
	//		// Orange mask
	//		inRange(faceScans[i], Scalar(11, 227, 50), Scalar(16, 255, 121), faceMasks[l]);
	//		dilate(faceMasks[l], faceMasks[l], kernel);
	//		// Blue mask
	//		inRange(faceScans[i], Scalar(93, 213, 0), Scalar(122, 255, 132), faceMasks[l]);
	//		dilate(faceMasks[l], faceMasks[l], kernel);
	//		// Red mask
	//		inRange(faceScans[i], Scalar(0, 151, 26), Scalar(7, 255, 95), faceMasks[l]);
	//		dilate(faceMasks[l], faceMasks[l], kernel);
	//		// Green mask
	//		inRange(faceScans[i], Scalar(50, 144, 16), Scalar(95, 236, 97), faceMasks[l]);
	//		dilate(faceMasks[l], faceMasks[l], kernel);
	//		// White mask
	//		inRange(faceScans[i], Scalar(93, 54, 128), Scalar(118, 102, 178), faceMasks[l]);
	//		dilate(faceMasks[l], faceMasks[l], kernel);
	//	}
