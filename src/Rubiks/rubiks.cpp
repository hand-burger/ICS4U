...
int main()
{
	...
	// String for user to input moves to turn the cube (When it isn't commented out)
	string function;
...
void solveYellowSide()
{
	// 1. Move any yellow edges out of the yellow side if they exist so that we don't need to check for them later if they're hiding up there
	moveYellowEdges();

	// 2. Solve the yellow edges
...
void twoEdgeRotate(int face, bool isNextTo)
{
	// Use the face and if they are together and a set of pre programmed moves to solve the edges
	if (face == 1)
	{
		if (isNextTo)
		{
